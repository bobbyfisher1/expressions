...
class ExpressionsValidator extends AbstractExpressionsValidator {
	...
	def private checkExpectedType(Expression exp, ExpressionsType expectedType, EReference reference) {
		val actualType = exp.getTypeAndCheckNotNull(reference)
		if (actualType != expectedType)
			error("expected " + expectedType + " type, but was " + actualType, reference, TYPE_MISMATCH)
	}

	def private ExpressionsType getTypeAndCheckNotNull(/*extension*/Expression exp, EReference reference) {
	...
	@Check def checkType(Not not) {
		checkExpectedBoolean(not.expression, ExpressionsPackage.Literals.NOT__EXPRESSION)
	}
	...
	@Check def checkType(Equality equality) {
		val leftType = equality.left.getTypeAndCheckNotNull(ExpressionsPackage.Literals.EQUALITY__LEFT)
		val rightType = equality.right.getTypeAndCheckNotNull(ExpressionsPackage.Literals.EQUALITY__RIGHT)
		checkExpectedSame(leftType, rightType)
	}

	@Check def checkType(Comparison comparison) {
		val leftType = comparison.left.getTypeAndCheckNotNull(ExpressionsPackage.Literals.COMPARISON__LEFT)
		val rightType = comparison.right.getTypeAndCheckNotNull(ExpressionsPackage.Literals.COMPARISON__RIGHT)
		checkExpectedSame(leftType, rightType)
	...
	@Check def checkType(Plus plus) {
		val leftType = plus.left.getTypeAndCheckNotNull(ExpressionsPackage.Literals.PLUS__LEFT)
		val rightType = plus.right.getTypeAndCheckNotNull(ExpressionsPackage.Literals.PLUS__RIGHT)
