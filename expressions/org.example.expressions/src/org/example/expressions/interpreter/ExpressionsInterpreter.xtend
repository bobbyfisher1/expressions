...
class ExpressionsInterpreter {
	...
	/* 
	 * For a BoolConstant object the feature value is also 
	 * of Java type String, and thus we perform an explicit 
	 * conversion using the static method of the Java class 
	 * Boolean.
	 */
	def dispatch Object interpret(Expression e) {
		switch (e) {
			IntConstant:
				e.value
			BoolConstant:
				Boolean.parseBoolean(e.value) // <---here
			StringConstant:
				e.value
			Not:
